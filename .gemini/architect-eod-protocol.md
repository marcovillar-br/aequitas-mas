...
1. Ensure the Claude Code (Developer) has successfully executed the `/eod` command in VS Code and generated the Diff Summary.
2. Open the GEM Web interface (with the Architect Persona loaded).
3. Paste the Developer's EOD Diff Summary.
4. Execute the "Internal Prompt" below to persist today's progress.

## 3. Internal Prompt (Copy and paste into GEM)
```markdown
Analyze the following Developer's EOD diff summary:
[INSERT VS CODE EOD DIFF SUMMARY HERE]
...
4. OUTPUT REQUIREMENT: Output the exact markdown changes and code blocks needed to persist today's progress in the documentation files. Do not hallucinate features that are not in the diff summary.
```
